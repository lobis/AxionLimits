...
def FilledLimit(ax, dat, text_label='', col='ForestGreen', edgecolor='k', zorder=1,
    ...
    if text_on:
        plt.text(text_pos[0], text_pos[1], text_label, fontsize=fs, color=text_col, ha=ha, va=va, clip_on=clip_on,
...
def FigSetup(xlab=r'$m_a$ [eV]', ylab='',
    ...
    ax.tick_params(which='major', direction=tickdir, width=2.5, length=13, right=TopAndRightTicks, top=TopAndRightTicks,
                   pad=tick_pad)
...
class AxionPhoton():
    def QCDAxion(ax, C_logwidth=10, KSVZ_on=True, DFSZ_on=True, cmap='YlOrBr', fs=18, RescaleByMass=False, text_on=True,
                 thick_lines=False, C_center=1, C_width=0.8, KSVZ_label_mass=1e-8, DFSZ_label_mass=5e-8, vmax=0.9):
        if RescaleByMass:
            rs1 = 1.0
            rs2 = 0.0
        else:
        ...
        if rs1 == 0:
            ...
            if KSVZ_on:
                ...
                if text_on:
                    plt.text(KSVZ_label_mass, g_x(KSVZ, KSVZ_label_mass) * 1.05, r'{\bf KSVZ}', fontsize=fs,
                             rotation=trans_angle, color=cols(1.0), ha='left', va='bottom', rotation_mode='anchor')
            if DFSZ_on:
                if thick_lines:
                ...
                if text_on:
                    plt.text(DFSZ_label_mass, g_x(DFSZ, DFSZ_label_mass) / 1.5, r'{\bf DFSZ}', fontsize=fs,
                             rotation=trans_angle, color=cols(1.0), ha='left', va='top', rotation_mode='anchor')
        else:
    ...
    def ADMX(ax, col=[0.8, 0.0, 0.0], projection=False, fs=15, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        plt.fill_between(dat[:, 0], dat[:, 1] / (rs1 * 2e-10 * dat[:, 0] + rs2), y2=y2, edgecolor=None, facecolor=col,
                         zorder=0.1)
        ...
        if projection:
            ...
            plt.fill_between(dat[:, 0], dat[:, 1] / (rs1 * 2e-10 * dat[:, 0] + rs2), y2=y2, edgecolor=None,
                             facecolor=col, zorder=0, alpha=0.1)
            if text_on:
                if rs1 == 0:
                    plt.text(1e-5 * text_shift[0], 2.3e-16 * text_shift[1], r'{\bf ADMX}', fontsize=20, color=col,
                             rotation=0, ha='left', va='top', clip_on=True)
                    plt.plot([3e-5, 2e-5], [3e-16, 0.6e-15], 'k-', lw=1.5)
                else:
                    plt.text(0.9e-6 * text_shift[0], 0.15 * text_shift[1], r'{\bf ADMX}', fontsize=fs, color=col,
                             rotation=0, ha='left', va='top', clip_on=True)
        else:
            if text_on:
                if rs1 == 0:
                    plt.text(0.85e-6 * text_shift[0], 1e-13 * text_shift[1], r'{\bf ADMX}', fontsize=fs, color=col,
    ...
    def RBF_UF(ax, col='darkred', fs=13, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        if text_on:
            if rs1 == 0:
                plt.text(text_shift[0] * 0.4e-5, text_shift[1] * 0.6e-11, r'{\bf RBF+UF}', fontsize=fs, color='w',
                         rotation=-90, ha='left', va='top', clip_on=True)
            else:
                plt.text(text_shift[0] * 0.7e-5, text_shift[1] * 4e3, r'{\bf RBF}', fontsize=fs, color='w', rotation=0,
                         ha='center', va='top', clip_on=True)
                plt.text(text_shift[0] * 0.7e-5, text_shift[1] * 1e3, r'{\bf UF}', fontsize=fs, color='w', rotation=0,
                         ha='center', va='top', clip_on=True)
    ...
    def HAYSTAC(ax, col=[0.88, 0.07, 0.37], fs=13, RescaleByMass=False, projection=True, text_on=True,
        ...
        if rs1 == 0:
            plt.plot([dat[0, 0], dat[0, 0]],
                     [dat[0, 1] / (rs1 * 2e-10 * dat[0, 0] + rs2), y2 / (rs1 * 2e-10 * dat[0, 0] + rs2)], color=col,
                     zorder=zo, lw=2)
            plt.plot([dat2[0, 0], dat2[0, 0]],
                     [dat2[0, 1] / (rs1 * 2e-10 * dat2[0, 0] + rs2), y2 / (rs1 * 2e-10 * dat2[0, 0] + rs2)], color=col,
                     zorder=zo, lw=2)

            if text_on:
                if projection == False:
                    plt.text(text_shift[0] * 2.1e-5, text_shift[0] * 5e-13, r'{\bf HAYSTAC}', fontsize=fs, color=col,
                             rotation=-90, ha='left', va='top', clip_on=True)
        else:
            plt.plot([dat[0, 0], dat[0, 0]],
                     [dat[0, 1] / (rs1 * 2e-10 * dat[0, 0] + rs2), y2 / (rs1 * 2e-10 * dat[0, 0] + rs2)], color='k',
                     zorder=zo, lw=4)
            plt.plot([dat[0, 0], dat[0, 0]],
                     [dat[0, 1] / (rs1 * 2e-10 * dat[0, 0] + rs2), y2 / (rs1 * 2e-10 * dat[0, 0] + rs2)], color=col,
                     zorder=zo, lw=3)
            plt.plot(dat[0, 0], dat[0, 1] / (rs1 * 2e-10 * dat[0, 0] + rs2), '.', markersize=15, color=col,
                     markeredgecolor='k', zorder=zo)

            plt.plot([dat2[0, 0], dat2[0, 0]],
                     [dat2[0, 1] / (rs1 * 2e-10 * dat2[0, 0] + rs2), y2 / (rs1 * 2e-10 * dat2[0, 0] + rs2)], color='k',
                     zorder=zo, lw=4)
            plt.plot([dat2[0, 0], dat2[0, 0]],
                     [dat2[0, 1] / (rs1 * 2e-10 * dat2[0, 0] + rs2), y2 / (rs1 * 2e-10 * dat2[0, 0] + rs2)], color=col,
                     zorder=zo, lw=3)
            plt.text(text_shift[0] * dat2[0, 0] * 1.1, text_shift[1] * y2 * 1.2, r'{\bf HAYSTAC}', fontsize=fs,
                     color=col, rotation=40, ha='left', rotation_mode='anchor')
            plt.plot(dat2[0, 0], dat2[0, 1] / (rs1 * 2e-10 * dat2[0, 0] + rs2), '.', markersize=15, color=col,
                     markeredgecolor='k', zorder=zo)
        return
    ...
    def CAPP(ax, col=[1, 0.1, 0.37], fs=15, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        if rs1 == 0:
            ...
            if text_on:
                plt.text(text_shift[0] * 0.8e-5, text_shift[1] * 0.1e-13, r'{\bf CAPP}', fontsize=fs, color=col,
            ...
            plt.plot([dat[0, 0], dat[0, 0]],
                     ...
                     zorder=zo, lw=3)
            if text_on:
                plt.text(text_shift[0] * dat[0, 0] * 1.1, text_shift[1] * y2 * 1.8, r'{\bf CAPP}', fontsize=fs,
                         color=col, rotation=40, ha='left', va='top', rotation_mode='anchor')
            plt.plot(dat[0, 0], dat[0, 1] / (rs1 * 2e-10 * dat[0, 0] + rs2), '.', markersize=15, color=col,
                     markeredgecolor='k', zorder=zo)
            imin = argmin(dat2[:, 1])
            plt.plot(dat2[imin, 0], dat2[imin, 1] / (rs1 * 2e-10 * dat2[0, 0] + rs2), '.', markersize=15, color=col,
                     markeredgecolor='k', zorder=zo)
            imin = argmin(dat3[:, 1])
            plt.plot(dat3[imin, 0], dat3[imin, 1] / (rs1 * 2e-10 * dat3[0, 0] + rs2), '.', markersize=15, color=col,
                     markeredgecolor='k', zorder=zo)
    ...
    def QUAX(ax, col='crimson', fs=15, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        if rs1 == 0:
            ...
            if text_on:
                plt.text(text_shift[0] * 6.3e-5, text_shift[1] * 0.8e-11, r'{\bf QUAX}', fontsize=fs, color=col,
                         rotation=-90, ha='center', va='top', clip_on=True)
        else:
            plt.plot([dat[0, 0], dat[0, 0]],
                     [dat[0, 1] / (rs1 * 2e-10 * dat[0, 0] + rs2), y2 / (rs1 * 2e-10 * dat[0, 0] + rs2)], color='k',
            ...
            plt.plot([dat2[0, 0], dat2[0, 0]],
                     ...
                     lw=3, zorder=zo)
            if text_on:
                plt.text(text_shift[0] * dat2[0, 0] * 1.2, text_shift[1] * y2 * 1.2, r'{\bf QUAX}', fontsize=fs,
                         color=col, rotation=40, ha='left', rotation_mode='anchor')
            plt.plot(dat[0, 0], dat[0, 1] / (rs1 * 2e-10 * dat[0, 0] + rs2), '.', markersize=15, color=col,
                     markeredgecolor='k', zorder=zo)
            plt.plot(dat2[0, 0], dat2[0, 1] / (rs1 * 2e-10 * dat2[0, 0] + rs2), '.', markersize=15, color=col,
    ...
    def ABRACADABRA(ax, col=[0.83, 0.07, 0.37], fs=15, projection=False, RescaleByMass=False, text_on=True, lw=1,
        ...
        if text_on:
            if rs1 == 0:
                plt.text(text_shift[0] * 1.5e-9, text_shift[1] * 3e-8, r'{\bf ABRA}', fontsize=fs, color='w',
                         rotation=0, ha='center', va='top', zorder=10, clip_on=True)
                plt.text(text_shift[0] * 1.5e-9, text_shift[1] * 1e-8, r'10 cm', fontsize=fs, color='w', rotation=0,
        ...
        if projection:
            ...
            if text_on:
                if rs1 == 0:
                    plt.text(text_shift[0] * 5e-12, text_shift[1] * 4e-18, r'{\bf ABRACADABRA}', fontsize=fs - 1,
                             color=col, rotation=13, ha='left', va='top', clip_on=True)
                else:
                    plt.text(text_shift[0] * 1.3e-9, text_shift[1] * 1.0e2, r'{\bf ABRACADABRA}', fontsize=fs - 1,
                             color=col, rotation=0, ha='left', va='top', clip_on=True)
                    plt.plot([dat[-1, 0], dat[-1, 0]], [dat[-1, 1] / (rs1 * 2e-10 * dat[-1, 0] + rs2), 1e6], lw=1.5,
                             color=col, zorder=0)
        return
    ...
    def DMRadio(ax, col=[0.83, 0.07, 0.37], fs=23, text_on=True, RescaleByMass=False, lw=2, text_shift=[1, 1]):
        ...
        if text_on:
            if rs1 == 0:
                plt.text(text_shift[0] * 2e-10, text_shift[1] * 0.5e-16, r'{\bf DM-Radio}', color='crimson',
                         fontsize=20, rotation=90, clip_on=True)
            else:
                plt.text(text_shift[0] * 5e-9, text_shift[1] * 4.0e-1, r'{\bf DM-Radio}', fontsize=fs - 1, color=col,
                         rotation=0, ha='left', va='top', clip_on=True)
        return
    ...
    def ORGAN(ax, col=[0.8, 0.0, 0.0], projection=False, fs=15, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        plt.fill_between(dat[:, 0], dat[:, 1] / (rs1 * 2e-10 * dat[:, 0] + rs2), y2=y2, edgecolor=col, facecolor=col,
                         zorder=0.1, lw=2)
        ...
        if projection:
            ...
            plt.plot(dat[:, 0], dat[:, 1] / (rs1 * 2e-10 * dat[:, 0] + rs2), '-', linewidth=1.5, color=col, zorder=0)
            plt.fill_between(dat[:, 0], dat[:, 1] / (rs1 * 2e-10 * dat[:, 0] + rs2), y2=y2, edgecolor=None,
                             facecolor=col, zorder=0, alpha=0.2)
            if text_on:
                if rs1 == 0:
                    plt.text(text_shift[0] * 5e-4, text_shift[1] * 1.15e-14, r'{\bf ORGAN}', fontsize=18, color=col,
                             rotation=0, ha='left', va='top', clip_on=True)
                    plt.plot([5e-4, 1.5e-4], [1.3e-14, 6e-13], 'k-', lw=1.5)
                else:
                    plt.text(text_shift[0] * 1.2e-4, text_shift[1] * 1e3, r'{\bf ORGAN}', fontsize=18, color='darkred',
                             rotation=-90, ha='left', va='top', clip_on=True)
            ...
            if text_on:
                if rs1 == 0:
                    plt.text(text_shift[0] * 110e-6, text_shift[1] * 1e-11, r'{\bf ORGAN}', fontsize=fs, color=col,
                             rotation=-90, ha='left', va='top', clip_on=True)
                else:
                    plt.text(text_shift[0] * dat[0, 0] * 1.1, text_shift[1] * y2 * 1.2, r'{\bf ORGAN}', fontsize=fs - 3,
                             color=col, rotation=40, ha='left', rotation_mode='anchor')
    ...
    def RADES(ax, col='blueviolet', fs=15, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        if rs1 == 0:
            ...
            plt.plot([dat[0, 0], dat[0, 0]],
                     ...
                     lw=3, zorder=zo)
            if text_on:
                plt.text(text_shift[0] * dat[0, 0] * 0.88, text_shift[1] * y2 * 1.2, r'{\bf RADES}', fontsize=fs,
                         color=col, rotation=40, ha='left', rotation_mode='anchor')
            plt.plot(dat[0, 0], dat[0, 1] / (rs1 * 2e-10 * dat[0, 0] + rs2), '.', markersize=15, color=col,
                     markeredgecolor='k', zorder=zo)
    ...
    def MADMAX(ax, col='darkred', fs=18, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        if text_on:
            if rs1 == 0:
                plt.text(text_shift[0] * 1.5e-4, text_shift[1] * 3.5e-15, r'{\bf MADMAX}', fontsize=18, color=col,
                         rotation=0, ha='left', va='top', clip_on=True)
                plt.plot([3e-4, 1.3e-4], [4.5e-15, 2.6e-14], 'k-', lw=1.5)
            else:
                plt.text(text_shift[0] * 5e-5, text_shift[1] * 3.5e0, r'{\bf MADMAX}', fontsize=14, color=col,
                         rotation=0, ha='left', va='top', clip_on=True)
    ...
    def ALPHA(ax, col='darkred', fs=18, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        if text_on:
            if rs1 == 0:
                plt.text(text_shift[0] * 1.5e-4, text_shift[1] * 1e-15, r'{\bf ALPHA}', fontsize=18, color=col,
                         rotation=0, ha='left', va='top', clip_on=True)
                plt.plot([1.3e-4, 0.5e-4], [1e-15, 0.7e-14], 'k-', lw=1.5)
            else:
                plt.text(text_shift[0] * 2.3e-4, text_shift[1] * 5e-1, r'{\bf ALPHA}', fontsize=fs, color=col,
    ...
    def KLASH(ax, col=[0.6, 0.1, 0.2], fs=15, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        plt.plot(dat[:, 0], dat[:, 1] / (rs1 * 2e-10 * dat[:, 0] + rs2), '-', linewidth=1.5, color=col, zorder=0)
        plt.fill_between(dat[:, 0], dat[:, 1] / (rs1 * 2e-10 * dat[:, 0] + rs2), y2=y2, facecolor=col, zorder=0,
                         alpha=0.3)
        if text_on:
            if rs1 == 0:
                plt.text(text_shift[0] * 1e-7, text_shift[1] * 1e-12, r'{\bf KLASH}', rotation=90, fontsize=fs,
                         color=col, ha='left', va='top', clip_on=True)
            else:
                plt.text(text_shift[0] * 2.5e-7, text_shift[1] * 1.3e0, r'{\bf KLASH}', rotation=90, fontsize=fs,
                         color=col, ha='left', va='top', rotation_mode='anchor', clip_on=True)
        return
    ...
    def BRASS(ax, col=[0.5, 0.1, 0.2], fs=15, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        if text_on:
            if rs1 == 0:
                plt.text(text_shift[0] * 1e-3, text_shift[1] * 0.5e-12, r'{\bf BRASS}', fontsize=14, ha='center',
                         rotation=48, color=col, clip_on=True)
            else:
                plt.text(text_shift[0] * 0.45e-3, text_shift[1] * 0.4e1, r'{\bf BRASS}', fontsize=20, rotation=9,
    ...
    def TOORAD(ax, col=[0.8, 0.1, 0.2], fs=15, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        if text_on:
            if rs1 == 0:
                plt.text(text_shift[0] * 0.7e-2, text_shift[1] * 3e-11, r'{\bf TOO}', fontsize=12, ha='center',
                         color=col, clip_on=True)
                plt.text(text_shift[0] * 0.7e-2, text_shift[1] * 1.5e-11, r'{\bf RAD}', fontsize=12, ha='center',
                         color=col, clip_on=True)
            else:
                plt.text(text_shift[0] * 0.25e-2, text_shift[1] * 0.3e2, r'{\bf TOORAD}', fontsize=18, rotation=-21,
                         color=col, clip_on=True)
        return
    ...
    def LAMPOST(ax, col=[0.8, 0.1, 0.2], fs=15, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        if text_on:
            if rs1 == 0:
                plt.text(text_shift[0] * 0.8e-1, text_shift[1] * 5e-12, r'{\bf LAMPOST}', rotation=-90, fontsize=fs,
                         color=col, ha='left', va='top', clip_on=True)
            else:
                plt.text(text_shift[0] * 0.9e-1, text_shift[1] * 1.9e-1, r'{\bf LAMPOST}', rotation=0, fontsize=fs,
                         color=col, ha='left', va='top', clip_on=True)
    ...
    def DANCE(ax, col=[0.8, 0.1, 0.2], fs=15, text_on=True, text_pos=[1.7e-12, 2e-13]):
        ...
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf DANCE}', rotation=50, fontsize=fs, color=col, ha='left', va='top',
                     clip_on=True)
        return
    ...
    def aLIGO(ax, col=[0.8, 0.1, 0.2], fs=15, text_on=True, text_pos=[0.2e-9, 0.35e-13]):
        ...
        plt.fill_between(dat[:, 0], dat[:, 1], y2=y2, facecolor=col, zorder=0, alpha=0.1)
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf aLIGO}', rotation=0, fontsize=fs, color=col, ha='left', va='top',
                     clip_on=True)
        return
    ...
    def ADBC(ax, col=[0.8, 0.1, 0.2], fs=15, text_on=True, text_pos=[1e-11, 1.3e-12], rotation=26):
        ...
        plt.fill_between(dat[:, 0], dat[:, 1], y2=y2, facecolor=col, zorder=0, alpha=0.1)
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf ADBC}', rotation=rotation, fontsize=fs, color=col, ha='left',
                     va='top', clip_on=True)
        return
    ...
    def SHAFT(ax, col='red', fs=16, text_on=True, lw=1, text_pos=[0.8e-10, 3e-10], rotation=0, zorder=1.8):
        ...
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf SHAFT}', fontsize=fs, color='w', rotation=rotation, ha='center',
                     va='top', clip_on=True)
        return
    ...
    def BASE(ax, col='crimson', fs=15, RescaleByMass=False, text_on=True, text_shift=[1, 1], zorder=1.9, lw=2.5):
        ...
        if rs1 == 0:
            plt.plot([dat[0, 0], dat[0, 0]],
                     ...
                     lw=lw, zorder=zo)
            if text_on:
                plt.text(text_shift[0] * 3e-9, text_shift[1] * 1.e-12, r'{\bf BASE}', fontsize=fs, color=col,
                         rotation=90, ha='center', va='top', clip_on=True)
        else:
            plt.plot([dat[0, 0], dat[0, 0]],
                     [dat[0, 1] / (rs1 * 2e-10 * dat[0, 0] + rs2), y2 / (rs1 * 2e-10 * dat[0, 0] + rs2)], color='k',
            ...
            plt.plot([dat[0, 0], dat[0, 0]],
                     ...
                     lw=lw + 1, zorder=zo)
            if text_on:
                plt.text(text_shift[0] * dat[0, 0] * 1.2, text_shift[1] * y2 * 1.2, r'{\bf BASE}', fontsize=fs,
                         color=col, rotation=40, ha='left', rotation_mode='anchor')
            plt.plot(dat[0, 0], dat[0, 1] / (rs1 * 2e-10 * dat[0, 0] + rs2), '.', markersize=15, color=col,
                     markeredgecolor='k', zorder=zo)
    ...
    def ADMX_SLIC(ax, col='crimson', fs=12, RescaleByMass=False, text_on=True, text_shift=[1, 1], zorder=1):
        ...
        if rs1 == 0:
            ...
            if text_on:
                plt.text(text_shift[0] * 2.4e-7, text_shift[1] * 0.8e-11, r'{\bf ADMX SLIC}', fontsize=fs, color=col,
                         rotation=-90, ha='center', va='top', clip_on=True)
        else:
            ...
            if text_on:
                plt.text(text_shift[0] * x, text_shift[1] * y2 * 1.2, r'{\bf ADMX SLIC}', fontsize=fs, color=col,
                         rotation=40, ha='left', rotation_mode='anchor')
            plt.plot(x, y / (rs1 * 2e-10 * x + rs2), '.', markersize=15, color=col, markeredgecolor='k', zorder=zo)

        return

    def ALPS(ax, projection=True, col=[0.8, 0.25, 0.33], fs=15, RescaleByMass=False, text_on=True, lw_proj=1.5,
             lsty_proj='-', col_proj='k', text_shift_x=1, text_shift_y=1):
        ...
        plt.fill_between(dat[:, 0], dat[:, 1] / (rs1 * 2e-10 * dat[:, 0] + rs2), y2=y2, edgecolor=None, facecolor=col,
                         zorder=1.53, lw=0.01)
        if rs1 == 0:
            if text_on: plt.text(1e-5 * text_shift_x, 7e-8 * text_shift_y, r'{\bf ALPS-I}', fontsize=20, color='w',
                                 clip_on=True)
        if projection:
            dat = loadtxt("limit_data/AxionPhoton/Projections/ALPS-II.txt")
            plt.plot(dat[:, 0], dat[:, 1] / (rs1 * 2e-10 * dat[:, 0] + rs2), linestyle=lsty_proj, lw=lw_proj,
                     zorder=1.5, color=col_proj, alpha=0.5)
            if RescaleByMass:
                plt.text(9e-4 * text_shift_x, 2.5e3 * text_shift_y, r'{\bf ALPS-II}', fontsize=20, color='k',
                         rotation=20, alpha=0.5, clip_on=True)
            else:
                if text_on: plt.text(1.5e-3 * text_shift_x, 3e-9 * text_shift_y, r'{\bf ALPS-II}', rotation=60,
                                     fontsize=18, color='w', zorder=10, clip_on=True)
        return

    def OSQAR(ax, text_label=r'{\bf OSQAR}', text_pos=[1e-5, 3e-8], col=[0.6, 0.2, 0.25], text_col='w', fs=17,
              zorder=1.52, text_on=True, edgealpha=1, lw=2):
        # PVLAS arXiv:[]
        dat = loadtxt("limit_data/AxionPhoton/OSQAR.txt")
        FilledLimit(ax, dat, text_label, text_pos=text_pos, col=col, text_col=text_col, fs=fs, zorder=zorder,
                    text_on=text_on, edgealpha=edgealpha, lw=lw)
        return

    def PVLAS(ax, text_label=r'{\bf PVLAS}', text_pos=[2e-3, 9e-8], col=[0.4, 0.2, 0.2], text_col='w', fs=17,
              zorder=1.51, text_on=True, edgealpha=1, rotation=45, lw=2):
        # PVLAS arXiv:[]
        dat = loadtxt("limit_data/AxionPhoton/PVLAS.txt")
        FilledLimit(ax, dat, text_label, text_pos=text_pos, col=col, text_col=text_col, fs=fs, zorder=zorder,
                    text_on=text_on, edgealpha=edgealpha, rotation=rotation, lw=lw)
        return

    def CROWS(ax, text_label=r'{\bf CROWS}', text_pos=[1e-7, 2.5e-7], col=[0.7, 0.2, 0.2], text_col='w', fs=17,
              zorder=1.54, text_on=True, edgealpha=1, lw=2):
    ...
    def Helioscopes(ax, col=[0.5, 0.0, 0.13], fs=25, projection=False, RescaleByMass=False, text_on=True):
        ...
        plt.fill_between(dat[:, 0], dat[:, 1] / (rs1 * 2e-10 * dat[:, 0] + rs2), y2=y2, edgecolor='k', facecolor=col,
                         zorder=1.49, lw=0.1)
        mf = dat[-2, 0]
        gf = dat[-2, 1]
        ...
        if projection:
            ...
            plt.plot(IAXO[:, 0], IAXO[:, 1] / (rs1 * 2e-10 * IAXO[:, 0] + rs2), '--', linewidth=2.5, color=IAXO_col,
                     zorder=0.5)
            plt.fill_between(IAXO[:, 0], IAXO[:, 1] / (rs1 * 2e-10 * IAXO[:, 0] + rs2), y2=y2, edgecolor=None,
                             facecolor=IAXO_col, zorder=0, alpha=0.3)
            if text_on == True:
                if rs1 == 0:
    ...
    def FermiSNe(ax, text_label=r'{\bf Fermi-SNe}', text_pos=[1.2e-12, 0.51e-10], col='ForestGreen', text_col='w',
                 fs=12, zorder=0.25, text_on=True):
    ...
    def DSNALP(ax, text_label=r'{\bf DSNALP}', text_pos=[1.2e-12, 1.2e-10], col=[0.0, 0.62, 0.3], text_col='w', fs=12,
               zorder=0.25, text_on=True):
    ...
    def SN1987A_gamma(ax, text_label=r'{\bf SN1987A}', text_pos=[6e-11, 0.4e-11], col='#067034', text_col='#067034',
                      fs=15, zorder=0.21, text_on=True):
    ...
    def Hydra(ax, text_label=r'{\bf Hydra}', text_pos=[1.2e-12, 2e-11], col=[0.24, 0.71, 0.54], text_col='w', fs=13,
              zorder=0.23, text_on=True):
    ...
    def M87(ax, text_label=r'\quad {\bf M87}', text_pos=[1.4e-12, 5e-12], col='seagreen', text_col='w', fs=15,
            zorder=0.219, text_on=True):
    ...
    def HESS(ax, text_label=r'{\bf HESS}', text_pos=[2e-8, 1.8e-11], col=[0.0, 0.55, 0.3], text_col=[0.0, 0.55, 0.3],
             fs=16, zorder=0.2, text_on=True):
    ...
    def Mrk421(ax, text_label=r'{\bf Mrk 421}', text_pos=[3e-9, 6e-11], col=[0.4, 0.6, 0.1], text_col='w', fs=12,
               zorder=0.26, text_on=True):
    ...
    def Chandra(ax, text_label=r'{\bf Chandra}', text_pos=[1.01e-11, 1.5e-12], col=[0.0, 0.3, 0.24],
                text_col=[0.0, 0.3, 0.24], fs=15, zorder=0.1, text_on=True):
    ...
    def Fermi(ax, text_label=r'{\bf Fermi}', text_pos=[4.02e-10, 1.2e-11], col=[0.0, 0.42, 0.24], text_col='w', fs=15,
              zorder=0.24, text_on=True):
        ...
        if text_on:
            plt.text(text_pos[0], text_pos[1], text_label, fontsize=fs, color=text_col, ha='left', va='top',
                     clip_on=True)
        return

    def StarClusters(ax, text_pos=[2.2e-11, 2.7e-11], col=[0.2, 0.54, 0.01], text_col='w', fs=13, zorder=0.22,
                     rotation=45, text_on=True):
        ...
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf Star}', fontsize=fs, color=text_col, ha='left', va='top',
                     rotation=rotation, clip_on=True)
            plt.text(0.88 * text_pos[0], text_pos[1], r'{\bf clusters}', fontsize=fs, color=text_col, ha='left',
                     va='top', rotation=rotation, clip_on=True)
        return

    def Fermi_GalacticSN(ax, text_label=r'{\bf Fermi SN}', text_pos=[1e-9, 5e-13], col=[0.0, 0.42, 0.24],
                         text_col=[0.0, 0.42, 0.24], fs=15, zorder=0.1, text_on=True, rotation=43, lw=1.5,
                         facealpha=0.2):
        # Fermi nearby SN prospects arXiv:[1609.02350]
        dat = loadtxt("limit_data/AxionPhoton/Projections/FermiSN.txt")
        FilledLimit(ax, dat, text_label, text_pos=text_pos, col=col, text_col=text_col, fs=fs, zorder=zorder,
                    text_on=text_on, lw=lw, rotation=rotation, facealpha=facealpha)
        return

    def MUSE(ax, text_label=r'{\bf MUSE}', text_pos=[1.5, 0.7e-12], col=[0.09, 0.45, 0.27], text_col=[0.09, 0.45, 0.27],
             fs=15, zorder=0.2, text_on=True):
        # Telescopes (MUSE) [2009.01310]
        dat = loadtxt("limit_data/AxionPhoton/Telescopes_MUSE.txt")
        FilledLimit(ax, dat, text_label, text_pos=text_pos, col=col, edgecolor=col, text_col=text_col, fs=fs,
                    zorder=zorder, text_on=text_on, rotation=90, lw=0.0, edgealpha=0)
        return

    def VIMOS(ax, text_label=r'{\bf VIMOS}', text_pos=[15, 1e-11], col=[0.09, 0.6, 0.27], text_col=[0.09, 0.6, 0.27],
              fs=15, zorder=0.2, text_on=True):
    ...
    def THESEUS(ax, text_label=r'{\bf THESEUS}', text_pos=[8e2, 0.8e-17], col=[0.03, 0.57, 0.82],
                edgecolor=[0.03, 0.57, 0.82], text_col=[0.03, 0.57, 0.82], fs=17, zorder=0.01, text_on=True, lw=2,
                facealpha=0.1):
        # THESEUS 2008.08306
    ...
    def eROSITA(ax, text_label=r'{\bf eROSITA}', text_pos=[2e3, 0.3e-18], col=[0.03, 0.57, 0.82],
                edgecolor=[0.03, 0.57, 0.82], text_col=[0.03, 0.57, 0.82], fs=17, zorder=0.01, text_on=True, lw=2,
                facealpha=0.1):
        # eROSITA 2103.13241
    ...
    def Cosmology(ax, fs=30, text_on=True):
        ...
        FilledLimit(ax, dat, r'{\bf X-rays}', text_pos=[1e4, 0.8e-16], col=[0.03, 0.57, 0.82], text_col='w', fs=fs,
                    zorder=0.05, text_on=text_on, rotation=-50, ha='left', va='top', edgealpha=0.8)

        # Extragalactic background light
        EBL = loadtxt("limit_data/AxionPhoton/EBL.txt")
        EBL2 = loadtxt("limit_data/AxionPhoton/EBL.txt")
        FilledLimit(ax, EBL, r'{\bf EBL}', text_pos=[5e4, 5e-14], col=[0.0, 0.2, 0.6], text_col='w', fs=fs + 5,
                    zorder=0.5, text_on=text_on, rotation=-55, ha='left', va='top', edgealpha=0.8, lw=2.5)
        ...
        if text_on:
            plt.text(100.5744 * 0.93, 5.1720e-11, r'{\bf Ionisation}', fontsize=fs - 9, color='w', rotation=-90,
                     ha='left', va='top', clip_on=True)
            plt.text(40 * 0.93, 4.1720e-11, r'{\bf fraction}', fontsize=fs - 9, color='w', rotation=-90, ha='left',
                     va='top', clip_on=True)

        # BBN+N_eff arXiv:[2002.08370]
        dat = loadtxt("limit_data/AxionPhoton/BBN_Neff.txt")
        FilledLimit(ax, dat, r'{\bf BBN}+$N_{\rm eff}$', text_pos=[2e5, 8e-10], col=[0.27, 0.51, 0.71], text_col='w',
                    fs=fs, zorder=0.4, text_on=text_on, rotation=-55, ha='left', va='top', edgealpha=0.5)
        return

    def HorizontalBranch(ax, text_label=r'{\bf Horizontal branch}', text_pos=[1.4e0, 1.5e-10], col=[0.0, 0.66, 0.42],
                         text_col='w', fs=23, zorder=1, text_on=True, lw=2):
        # Globular clusters arXiv:[1406.6053]
        dat = loadtxt("limit_data/AxionPhoton/HorizontalBranch.txt")
        FilledLimit(ax, dat, text_label, text_pos=text_pos, col=col, text_col=text_col, fs=fs, zorder=zorder,
                    text_on=text_on, lw=lw, va='center')
        return

    def SolarNu(ax, text_label=r'{\bf Solar} $\nu$', text_pos=[1e1, 3e-9], col='seagreen', text_col='w', fs=33,
                zorder=1, text_on=True, lw=2):
    ...
    def SN1987A_decay(ax, text_label=r'{\bf Solar} $\nu$', text_pos=[1e1, 3e-9], col='#067034', text_col='w', fs=21,
                      zorder=0.1, text_on=True, lw=2, rotation=-60):
        ...
        plt.fill_between(SN[:, 0], SN[:, 1], y2=1e0, edgecolor=None, facecolor=col, zorder=zorder)
        if text_on:
            plt.text(0.4e6, 6e-7, r'{\bf SN1987A}', fontsize=fs, color=text_col, rotation=rotation, ha='left', va='top',
                     clip_on=True)
        return
    ...
    def NeutronStars(ax, col=[0.1, 0.5, 0.2], fs=17, RescaleByMass=False, text_on=True, text_shift=[1, 1]):
        ...
        if text_on:
            if rs1 == 0:
                plt.text(text_shift[0] * 3e-6, text_shift[1] * 0.8e-10, r'{\bf Neutron stars}', fontsize=fs, color='w',
                         ha='left')
            else:
                plt.text(text_shift[0] * 1e-7, text_shift[1] * 4e3, r'{\bf Neutron}', fontsize=fs, color=col,
                         ha='center')
...
# ==============================================================================#
class AxionElectron():
    def QCDAxion(ax, C_logwidth=10, KSVZ_on=True, DFSZ_on=True, Hadronic_on=True, fs=25, DFSZ_col='goldenrod',
                 KSVZ_col='brown', Hadronic_col='gold', DFSZ_label_mass=1e-4, KSVZ_label_mass=0.8e-1,
                 Hadronic_label_mass=5e-3):
        ## QCD Axion band:
        g_min, g_max = ax.get_ylim()
        m_min, m_max = ax.get_xlim()
        ...
        if DFSZ_on:
            ...
            plt.text(DFSZ_label_mass, g_x(DFSZ_u, DFSZ_label_mass) / 1.5, r'{\bf DFSZ}', fontsize=fs,
                     rotation=trans_angle, color='k', ha='left', va='top', rotation_mode='anchor', clip_on=True)
        if KSVZ_on:
            col = KSVZ_col
            plt.plot(m, g_x(KSVZ, m), 'k-', lw=3.5, zorder=0.02)
            plt.plot(m, g_x(KSVZ, m), '-', lw=2, zorder=0.02, color=col)
            plt.text(KSVZ_label_mass, g_x(KSVZ, KSVZ_label_mass) * 2.7, r'{\bf KSVZ}', fontsize=fs,
                     rotation=trans_angle, color=col, ha='left', va='top', rotation_mode='anchor', clip_on=True)
        if Hadronic_on:
            col = Hadronic_col
            ...
            plt.text(Hadronic_label_mass, g_x(Had_u, Hadronic_label_mass) / 1.5, r'{\bf Hadronic models}',
                     fontsize=fs - 5, rotation=trans_angle, color='k', ha='left', va='top', rotation_mode='anchor',
                     clip_on=True)
    ...
    def XENON1T(ax, col='crimson', fs=19, text_on=True, zorder=0.51, text_shift=[1, 1], **kwargs):
        ...
        plt.fill_between(dat[:, 0], dat[:, 1], y2=1e0, edgecolor=None, facecolor=col, zorder=zorder)
        if text_on:
            plt.text(text_shift[0] * 1.2e2, text_shift[1] * 4e-14, r'{\bf XENON1T}', fontsize=fs, color=col,
                     ha='center', va='top', clip_on=True, **kwargs)
            plt.text(text_shift[0] * 1.2e2, text_shift[1] * 2.5e-14, r'(DM)', fontsize=fs, color=col, ha='center',
    ...
    def SolarBasin(ax, col='royalblue', fs=20, text_on=True, text_shift=[1, 1], zorder=0.6, **kwargs):
        ...
        plt.fill_between(dat[:, 0], dat[:, 1], y2=1e0, edgecolor=None, facecolor=col, zorder=zorder)
        if text_on:
            plt.text(text_shift[0] * 3e3, text_shift[1] * 2e-11, r'{\bf XENON1T}', fontsize=fs, color='k', ha='center',
                     va='top', clip_on=True, **kwargs)
            plt.text(text_shift[0] * 3e3, text_shift[1] * 1.3e-11, r'(Solar axion', fontsize=fs, color='k', ha='center',
                     va='top', clip_on=True, **kwargs)
            plt.text(text_shift[0] * 3e3, text_shift[1] * 0.8e-11, r' basin)', fontsize=fs, color='k', ha='center',
                     va='top', clip_on=True, **kwargs)
        return
    ...
    def LUX(ax, col='indianred', fs=30, text_on=True, text_pos=[0.2e-8, 6e-12], zorder=0.52, **kwargs):
        ...
        plt.fill_between(dat[:, 0], dat[:, 1], y2=1e0, edgecolor=None, facecolor=col, zorder=zorder)
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf LUX} (Solar axions)', fontsize=fs, color='w', alpha=0.8,
                     ha='left', va='top', clip_on=True, **kwargs)
        return
    ...
    def PandaX(ax, col='firebrick', fs=20, text_on=True, text_pos=[1.2e3, 4.5e-13], zorder=0.53, rotation=20, **kwargs):
        ...
        plt.fill_between(dat[:, 0], dat[:, 1], y2=1e0, edgecolor=None, facecolor=col, zorder=zorder)
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf PandaX}', fontsize=fs - 2, color='w', ha='left', va='top',
                     rotation=rotation, clip_on=True, **kwargs)
        return

    def EDELWEISS(ax, col='darkred', projection=False, fs=15, text_on=True, text_pos=[9e0, 7e-13], zorder=0.57,
                  rotation=0, **kwargs):
        ...
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf EDELWEISS}', fontsize=fs, color=col, ha='left', va='top',
                     clip_on=True, rotation=rotation, **kwargs)
        return

    def SuperCDMS(ax, col='maroon', fs=20, text_on=True, text_pos=[5e1, 2.7e-11], text_col='w', zorder=0.58,
                  rotation=-84, **kwargs):
        ...
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf SuperCDMS}', fontsize=fs - 1, color=text_col, ha='left', va='top',
                     alpha=1.0, rotation=rotation, clip_on=True, **kwargs)
        return
    ...
    def DARWIN(ax, col='brown', fs=20, text_on=True, text_pos=[0.3e3, 2e-14], zorder=0.1, **kwargs):
        ...
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf DARWIN}', fontsize=fs, color=col, ha='left', va='top',
                     clip_on=True, **kwargs)
        return
    ...
    def LZ(ax, col='crimson', fs=20, text_on=True, text_pos=[1.5e4, 2e-14], zorder=0.1, **kwargs):
        ...
        plt.plot(dat[:, 0], dat[:, 1], '--', color=col, alpha=1.0, zorder=zorder, lw=3)
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf LZ}', fontsize=fs, color=col, ha='left', va='top', clip_on=True,
                     **kwargs)
        return

    def Semiconductors(ax, col='purple', fs=15, text_on=True, text_pos=[1e0, 2.5e-12], rotation=-80, zorder=0.51,
                       **kwargs):
        ...
        plt.plot(dat[:, 0], dat[:, 1], '--', color=col, alpha=1.0, zorder=zorder, lw=3)
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf Semiconductors}', fontsize=fs, color=col, ha='left', va='top',
                     rotation=rotation, clip_on=True, **kwargs)
        return
    ...
    def MagnonScan(ax, col='mediumvioletred', fs=20, text_on=True, text_shift=[1, 1], zorder=0.5, **kwargs):
        ...
        plt.plot(dat[:, 0], dat[:, 1], '--', color=col, alpha=1.0, zorder=zorder, lw=3)
        if text_on:
            plt.text(text_shift[0] * 1.2e-5, text_shift[1] * 0.5e-13, r'{\bf Magnons}', fontsize=fs - 1, color=col,
                     ha='center', va='top', clip_on=True, **kwargs)
            plt.text(text_shift[0] * 1.2e-5, text_shift[1] * 0.7 * 0.5e-13, r'{\bf (Scanning)}', fontsize=fs - 1,
    ...
    def QUAX(ax, col='maroon', fs=17, text_on=True, text_pos=[50e-6, 0.9e-10], zorder=10.0, text_rot=-90, **kwargs):
        ...
        plt.plot(dat[:, 0], dat[:, 1], '-', color=col, alpha=1.0, zorder=zorder, lw=3)
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf QUAX}', fontsize=fs, color=col, rotation=text_rot, ha='left',
                     va='top', clip_on=True, **kwargs)
        return
    ...
    def RedGiants(ax, col=[0.0, 0.66, 0.42], text_pos=[0.2e-8, 2e-13], text_on=True, zorder=0.5, fs=30, **kwargs):
        ...
        plt.fill_between(dat[:, 0], dat[:, 1], y2=1e0, edgecolor=None, facecolor=col, zorder=zorder)
        if text_on: plt.text(text_pos[0], text_pos[1], r'{\bf Red giants} ($\omega$Cen)', fontsize=fs, color='w',
                             clip_on=True, **kwargs)
        return
...
class AxionNeutron():
    ...
    def QCDAxion(ax, C_logwidth=10, KSVZ_on=True, DFSZ_on=True,
        ...
        if KSVZ_on:
            ...
            plt.text(KSVZ_label_mass, g_x(KSVZ, KSVZ_label_mass) / 2, r'{\bf KSVZ}', fontsize=fs, rotation=trans_angle,
                     color='k', ha='left', va='top', rotation_mode='anchor', clip_on=True)
    ...
    def OldComagnetometers(ax, col=[0.75, 0.2, 0.2], fs=20, projection=True):
        ...
        plt.text(2e-20 * (1 - 0.07), 3e-5 * (1 + 0.07), r'{\bf Old comagnetometers}', fontsize=fs, color='k',
                 ha='center', va='top', rotation=-10, clip_on=True)
        ...
        if projection:
            ...
            plt.text(5e-18, 2 * 0.5e-12, r'{\bf Future comagnetometers}', fontsize=fs - 1, color=col, ha='left',
                     va='top', clip_on=True)
        return
    ...
    def UltracoldNeutronsAndMercury(ax, col=[0.5, 0.0, 0.13], fs=20, projection=True):
        ...
        plt.text(0.5e-19 * (1 - 0.07), 2 * StochasticCorrection * 2.5e-5 * (1 + 0.07), r'$\nu_n/\nu_{\rm Hg}$',
                 fontsize=fs, color='k', ha='left', va='top', clip_on=True)
        plt.text(0.5e-19, 2 * StochasticCorrection * 2.5e-5, r'$\nu_n/\nu_{\rm Hg}$', fontsize=fs, color='w', ha='left',
                 va='top', clip_on=True)
        return
    ...
    class CASPEr():
        def ZULF(ax, col=[0.6, 0.1, 0.1], fs=20, projection=True):
            ...
            plt.fill_between(dat[:, 0], StochasticCorrection * dat[:, 1], y2=y2, edgecolor=None, facecolor=col,
                             zorder=zo, alpha=1.0)
            plt.text(0.3e-16 * (1 - 0.05), (1 + 0.07) * StochasticCorrection * 0.95e-5, r'{\bf CASPEr-ZULF}',
                     fontsize=fs - 4, color='k', ha='left', va='top', rotation=40, rotation_mode='anchor', clip_on=True)
            plt.text(0.3e-16, StochasticCorrection * 0.95e-5, r'{\bf CASPEr-ZULF}', fontsize=fs - 4, color='w',
                     ha='left', va='top', rotation=40, rotation_mode='anchor', clip_on=True)
            if projection:
                ...
                plt.text(1e-22, 2 * 8e-11, r'{\bf CASPEr-ZULF} (projected)', fontsize=fs, color=col, ha='left',
                         va='top', clip_on=True)
            return
        ...
        def Comagnetometer(ax, col='darkred', fs=20, projection=True):
            ...
            plt.fill_between(dat[:, 0], dat[:, 1], y2=y2, edgecolor=None, facecolor=col, zorder=zo, alpha=1.0)
            plt.text(1e-21 * (1 - 0.05), (1 + 0.07) * 1.5 * 5e-3, r'{\bf CASPEr-comag.}', fontsize=fs - 1, color='k',
                     ha='left', va='top', clip_on=True)
        ...
        def wind(ax, col='red', fs=20, projection=True):
            ...
            plt.text(1.4e-9, 2 * 1.1e-11, r'{\bf CASPEr}-wind', fontsize=fs, color=col, ha='left', va='top',
                     rotation=27, clip_on=True)
            return
    ...
    def LabExperiments(ax, projection=True, fs=20):
        ...
        plt.plot(dat[:, 0], dat[:, 1], '-', color='k', alpha=1, zorder=zo, lw=3)
        plt.fill_between(dat[:, 0], dat[:, 1], y2=y2, edgecolor=None, facecolor=col, zorder=zo)
        plt.text((1 - 0.07) * 5.0e-12, (1 + 0.07) * 2.2e-4, r'{\bf K-}$^3${\bf He comagnetometer}', fontsize=fs,
                 color='k', ha='left', va='top', clip_on=True)
        ...
        plt.text((1 - 0.07) * 1e-8, (1 + 0.07) * 3e-3, r'{\bf Torsion balance}', fontsize=fs * 1.1, color='k',
                 ha='left', va='top', clip_on=True)
        ...
        plt.plot(dat[:, 0], dat[:, 1], '-', color='k', alpha=1, zorder=zo, lw=3)
        plt.fill_between(dat[:, 0], dat[:, 1], y2=y2, edgecolor=None, facecolor=col, zorder=zo)
        plt.text((1 - 0.07) * 0.8e-2, (1 + 0.07) * 3 * 1.6e-4, r'{\bf SNO}', fontsize=fs + 6, color='k', ha='right',
                 va='top', clip_on=True)
    ...
    def StellarBounds(ax, fs=30):
        ...
        plt.text((1 - 0.05) * 0.8e-2, (1 + 0.05) * 2 * 2e-8, r'{\bf SN1987A}', fontsize=fs, color='k', ha='right',
                 va='top', clip_on=True)
        ...
        plt.text((1 - 0.05) * 0.8e-2, (1 + 0.05) * 9e-10, r'{\bf Neutron star cooling}', fontsize=fs - 6, color='k',
...
class AxionProton():
    ...
    def QCDAxion(ax, C_logwidth=10, KSVZ_on=True, DFSZ_on=True,
        ...
        if KSVZ_on:
            ...
            plt.plot(m, g_x(KSVZ, m), 'k-', lw=3.5, zorder=0)
            plt.plot(m, g_x(KSVZ, m), '-', lw=2, zorder=0, color=col)
            plt.text(KSVZ_label_mass, g_x(KSVZ, KSVZ_label_mass) * 6, r'{\bf KSVZ}', fontsize=fs, rotation=trans_angle,
                     color='k', ha='left', va='top', rotation_mode='anchor', clip_on=True)
    ...
    class CASPEr():
        def ZULF(ax, col=[0.6, 0.1, 0.1], fs=20, projection=True):
            ...
            plt.fill_between(dat[:, 0], StochasticCorrection * dat[:, 1], y2=y2, edgecolor=None, facecolor=col,
                             zorder=zo, alpha=1.0)
            plt.text(0.3e-16 * (1 - 0.05), (1 + 0.07) * StochasticCorrection * 0.95e-5, r'{\bf CASPEr-ZULF}',
                     fontsize=fs - 4, color='k', ha='left', va='top', rotation=40, rotation_mode='anchor')
            plt.text(0.3e-16, StochasticCorrection * 0.95e-5, r'{\bf CASPEr-ZULF}', fontsize=fs - 4, color='w',
    ...
    def StellarBounds(ax, fs=30):
        ...
        plt.fill_between(SN[:, 0], SN[:, 1], y2=y2, edgecolor=None, facecolor='#067034', zorder=0.02)
        plt.plot(SN[:, 0], SN[:, 1], 'k-', alpha=1, lw=2.5, zorder=0.02)
        plt.text((1 - 0.05) * 0.8e-2, (1 + 0.05) * 1.9e-8, r'{\bf SN1987A}', fontsize=fs, color='k', ha='right',
                 va='top')
        ...
        plt.fill_between(SN[:, 0], SN[:, 1], y2=y2, edgecolor=None, facecolor='DarkGreen', zorder=0.01)
        plt.plot(SN[:, 0], SN[:, 1], 'k-', alpha=1, lw=2.5, zorder=0.01)
        plt.text((1 - 0.05) * 0.8e-2, (1 + 0.05) * 1.3e-9, r'{\bf Neutron star cooling}', fontsize=fs - 6, color='k',
                 ha='right', va='top')
...
class AxionEDM():
    ...
    def CASPEr(ax, text_pos=[130e-9, 1e-3], col='crimson', text_col='w', fs=20, zorder=30, projection=False,
        ...
        if text_on:
            plt.text(text_pos[0], text_pos[1], r'{\bf CASPEr-electric}', color=text_col, fontsize=fs, ha='right',
                     zorder=zorder)
        if projection:
...
class Axion_fa():
    ...
    def GW170817(ax, text_pos=[7e-16, 2e-17], zo=-7, linespacing_y=0.65, col=col_alpha('teal', 0.4), text_col='teal',
                 text_rot=0, fs=23):
        ...
        plt.text(text_pos[0], text_pos[1], r'{\bf GW170817}', color=text_col, rotation=text_rot, fontsize=fs,
                 ha='center', clip_on=True)
        return

    def Pulsars(ax, text_pos=[2e-15, 0.12e-16], linespacing_y=0.65, col='#05526e', text_col='#05526e', text_rot=0,
                fs=21, zo=-6.9):
        dat = loadtxt('limit_data/fa/Pulsar.txt')
        plt.plot(dat[:, 0], dat[:, 1], color='k', lw=3, alpha=1, zorder=zo)
        plt.fill_between(dat[:, 0], dat[:, 1], color=col, zorder=zo, alpha=1)

        plt.text(text_pos[0], text_pos[1] * (1 - linespacing_y), r'{\bf Pulsars}', color=text_col, rotation=text_rot,
    ...
    def NeutronStars(ax, text_pos=[0.5e-3, 1.1e-12], col='#1f6ff0', text_col='#1f6ff0', text_rot=43.5, fs=29,
                     zorder=-10):
        ...
        plt.text(text_pos[0], text_pos[1], r'{\bf Neutron stars}', color=text_col, rotation=text_rot, fontsize=fs,
